...
const pathToPage = (path: string) => {
  ...
  if (normalized === '/customer-stories') return 'stories';
  if (normalized === '/blog') return 'blog';
  if (normalized.startsWith('/blog/')) return `blog/${normalized.replace('/blog/', '')}`;
...
const pageToPath = (page: string) => {
  ...
  if (page === 'stories') return '/customer-stories';
...
const App: React.FC = () => {
  ...
  const { blogPosts, landingPages } = useCmsData();

  // Simple URL-based routing to check for dashboard
  ...
  useEffect(() => {
    const handlePopState = () => {
      ...
      setIsDashboard(route === 'dashboard');
      if (route !== 'dashboard') {
  ...
  const isLandingPage = currentPage.startsWith('landing/');

  useEffect(() => {
    // Only show popup on home page
    if (currentPage === 'home' && !isLandingPage) {
  ...
  const renderPage = () => {
    ...
    if (currentPage.startsWith('blog/')) {
      const blogSlug = currentPage.split('/')[1];
      const isPreview = new URLSearchParams(window.location.search).get('preview') === '1';
      const post = blogPosts.find(
        (item) => item.slug === blogSlug && ((item.status || 'published') === 'published' || isPreview)
      );
      if (post) {
        const relatedPosts = blogPosts
          ...
          .slice(0, 3);
        return <BlogPostTemplate post={post} relatedPosts={relatedPosts} onNavigate={navigate} />;
      }
    }

    if (currentPage.startsWith('landing/')) {
      const landingSlug = currentPage.split('/')[1];
  ...
  return (
    <div className="min-h-screen bg-white text-gray-800 relative">
      {!isLandingPage && <Header onNavigate={navigate} />}

      <main>
        {renderPage()}
      </main>

      {!isLandingPage && <Footer onNavigate={navigate} />}
      {!isLandingPage && <MobileBottomBar onNavigate={navigate} />}

      {!isLandingPage && showPopup && <PopupModal onClose={() => setShowPopup(false)} />}
